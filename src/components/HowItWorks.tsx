    ...
    <section className="py-24 bg-secondary/30">
      <div className="container mx-auto px-4">
            ...
            <Card key={index} className="bg-card border border-border shadow-card hover:shadow-corporate transition-all duration-300 group">
              <CardContent className="p-8 text-center">
                <div className="mb-6 relative">
                  <div className={`w-16 h-16 mx-auto rounded-lg flex items-center justify-center transition-transform duration-300 ${
                    step.color === 'web3-primary' ? 'bg-web3-primary/10 text-web3-primary' :
                    step.color === 'web3-secondary' ? 'bg-web3-secondary/10 text-web3-secondary' :
                    step.color === 'web3-success' ? 'bg-web3-success/10 text-web3-success' :
                    'bg-web3-warning/10 text-web3-warning'
                  }`}>
                    {step.icon}
                  </div>
                  <div className="absolute -top-2 -right-2 w-8 h-8 bg-web3-primary rounded-full flex items-center justify-center text-white text-sm font-bold">
                    {index + 1}
                  </div>
                </div>
        ...
        <div className="text-center mt-16">
          <div className="inline-flex items-center space-x-2 bg-web3-primary/10 rounded-full px-6 py-3">
            <Lock className="w-5 h-5 text-web3-primary" />
            <span className="text-web3-primary font-medium">Powered by Solana Smart Contracts</span>
          </div>
        </div>
      </div>
    </section>
